...
class DatabaseManager:
    ...
    def query_nutrition_data(self, ingredient_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            exact_match = self.nutrition_df[self.nutrition_df['name'] == normalized_name]
            ...
            for _, row in self.nutrition_df.iterrows():
                ...
                if score > best_match_score and score >= 60: # 60% confidence threshold
    ...
    def query_vegan_ontology(self, ingredient_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            exact_match_term = self.vegan_ontology_df[self.vegan_ontology_df['term'] == normalized_name]
            if not exact_match_term.empty:
                result = exact_match_term.iloc[0].to_dict()
                result['is_vegan_term'] = not bool(result.get('is_explicitly_non_vegan', False))
                logger.debug(f"Found vegan info for '{ingredient_name}' (exact term match): {result}")
                return result

            # Try exact match in 'aliases' column (if aliases are pre-parsed lists)
            alias_match = self.vegan_ontology_df[
                self.vegan_ontology_df['aliases'].apply(lambda x: normalized_name in x if isinstance(x, list) else False)
            ]
            if not alias_match.empty:
                result = alias_match.iloc[0].to_dict()
                result['is_vegan_term'] = not bool(result.get('is_explicitly_non_vegan', False))
                logger.debug(f"Found vegan info for '{ingredient_name}' (exact alias match): {result}")
                return result
            ...
            for _, row in self.vegan_ontology_df.iterrows():
                ...
                if isinstance(row['aliases'], list):
                    for alias in row['aliases']:
                        score_alias = max(score_alias, fuzz.ratio(normalized_name, str(alias)))

                combined_score = max(score_term, score_alias)

                if combined_score > best_match_score and combined_score >= 60: # 60% confidence threshold
                    best_match_score = combined_score
                    best_match_data = row.to_dict()

            if best_match_data:
                best_match_data['is_vegan_term'] = not bool(best_match_data.get('is_explicitly_non_vegan', False))
                logger.debug(f"Found vegan info for '{ingredient_name}' (fuzzy match, score {best_match_score:.2f}): {best_match_data}")
                return best_match_data
            else:
                logger.debug(f"No vegan info found for '{ingredient_name}' after exact and fuzzy search.")
        except Exception as e:
            logger.error(f"Failed to query vegan ontology for {ingredient_name}: {e}")
        return None
    
    def search_recipes(self, ingredient_query: str, size: int = 100000) -> Dict[str, Any]:
        """
