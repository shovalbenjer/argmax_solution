...
def test_evaluation_pipeline(monkeypatch, mock_mlflow_eval, tmp_path):
    ...
    # 4. Assertions
    # Verify MLflow calls
    evaluation_main.mlflow.log_param.assert_any_call("ground_truth_run_id", "test_gt_run_id")
    evaluation_main.mlflow.log_metric.assert_any_call("accuracy_numba", 0.5)
    evaluation_main.mlflow.log_metric.assert_any_call("f1_score_weighted", pytest.approx(0.4, 0.1))
