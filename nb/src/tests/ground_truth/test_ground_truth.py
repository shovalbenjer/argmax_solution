...
def test_ground_truth_pipeline(monkeypatch, mock_mlflow, tmp_path):
    ...
    monkeypatch.setattr(
        "polars.read_csv", 
        MagicMock(return_value=mock_recipe_df)
    )
    ...
    mock_api_response = '{"persona": "not_keto_or_vegan", "reasoning": "Test reason.", "citations": []}'
    monkeypatch.setattr(
    ...
    ground_truth_main.mlflow.set_experiment.assert_called_with("Ground Truth Generation")
    ground_truth_main.mlflow.log_param.assert_any_call("num_recipes", 1)
    ground_truth_main.mlflow.log_metric.assert_called_with("successfully_processed_recipes", 1)
