...
### 0. `00_run_all_tests.py` - System Health Check (NEW)
**Purpose:** Comprehensive system test to ensure all components are working before running the main pipeline.

**What it does:**
- Tests database connectivity and required tables
- Verifies LLM service availability
- Tests ingredient parser functionality
- Tests GeneralizedIngredientAnalyzer components
- Validates configuration loading
- Provides detailed test results and recommendations

**Usage:**
```bash
python scripts/00_run_all_tests.py
```

**Output:** Test summary with pass/fail status for each component.
...
### 6. `06_run_task_notebook.py` - Task Notebook Pipeline (NEW)
**Purpose:** Runs the complete task.ipynb pipeline programmatically, executing all evaluation and analysis components.

**What it does:**
- Runs final evaluation against ground truth data
- Executes evaluation analysis and reporting
- Runs comprehensive performance testing
- Tests deepeval pipeline functionality
- Provides complete pipeline status and results

**Usage:**
```bash
python scripts/06_run_task_notebook.py
```

**Prerequisites:** All previous scripts (00-05) must have completed successfully.
