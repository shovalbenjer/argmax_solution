...
from sqlalchemy import (JSON, Boolean, Column, Float, Integer, MetaData,
...
def create_database_schema():
    ...
    This function drops existing tables and creates fresh schema with
    ...
    with engine.connect() as connection:
        connection.execute(
            text(
                "CREATE INDEX IF NOT EXISTS idx_nutrition_name ON nutrition_facts (name);"
            )
        )
        connection.execute(
            text("CREATE INDEX IF NOT EXISTS idx_vegan_term ON vegan_ontology (term);")
        )
        connection.execute(
            text("CREATE INDEX IF NOT EXISTS idx_unit_name ON unit_conversions (unit);")
        )
        connection.commit()
...
def clean_numeric_value(value_str):
    ...
    Args:
        value_str: Raw string value from CSV
        
    Returns:
        float: Cleaned numeric value, defaults to 0.0 if invalid
    """
...
def ingest_nutrition_data():
    ...
    This function processes the nutrition.csv file, cleans numeric values,
    maps column names to database schema, and inserts data into the
    nutrition_facts table.
    
    Raises:
        FileNotFoundError: If nutrition.csv is missing
        ValueError: If data validation fails
    ...
    try:
        ...
        expected_cols = {
            c.name for c in nutrition_facts_table.columns if c.name != "id"
        }
        ...
        for col in expected_cols:
            if col not in df_filtered.columns:
                df_filtered[col] = (
                    0.0
                    if nutrition_facts_table.c[col].type.python_type == float
                    else ""
                )
        ...
        with engine.connect() as connection:
            df_filtered.to_sql(
                "nutrition_facts", connection, if_exists="append", index=False
            )
...
def ingest_vegan_ontology():
    ...
    This function processes the vegan_ontology.csv file, parses JSON aliases,
    converts boolean flags, and inserts data into the vegan_ontology table.
    
    Raises:
        FileNotFoundError: If vegan_ontology.csv is missing
        ValueError: If data validation fails
        sqlalchemy.exc.SQLAlchemyError: If database insertion fails
    """
    ...
    try:
        ...
        def parse_aliases(x):
            if isinstance(x, str) and x.startswith("["):
                try:
                    return json.dumps(
                        [str(item).lower().strip() for item in ast.literal_eval(x)]
                    )
        ...
        for col in [
            ...
            "requires_contextual_check",
            "is_vegan_exception_term",
        ]:
        ...
        with engine.connect() as connection:
            df_filtered.to_sql(
                "vegan_ontology", connection, if_exists="append", index=False
            )
        logger.success(
            f"Successfully ingested {len(df_filtered)} vegan ontology terms."
        )
    except Exception as e:
...
def ingest_unit_conversions():
    ...
    This function processes the unit_conversion.csv file, standardizes
    column names, converts factors to numeric values, and inserts data
    into the unit_conversions table.
    
    Raises:
        FileNotFoundError: If unit_conversion.csv is missing
        ValueError: If data validation fails
        sqlalchemy.exc.SQLAlchemyError: If database insertion fails
    """
    ...
    try:
        ...
        expected_cols = {
            c.name for c in unit_conversions_table.columns if c.name != "id"
        }
        df_filtered = df[[col for col in df.columns if col in expected_cols]]
        ...
        with engine.connect() as connection:
            df_filtered.to_sql(
                "unit_conversions", connection, if_exists="append", index=False
            )
        logger.success(
            f"Successfully ingested {len(df_filtered)} unit conversion records."
        )
    except Exception as e:
...
def main():
    ...
    2. Ingests nutrition facts data
    3. Ingests vegan ontology data
    4. Ingests unit conversion data
    
    The function handles database recreation and provides comprehensive
    ...
    if DB_PATH.exists():
        logger.warning(
            f"Database at {DB_PATH} already exists. Deleting for a fresh start."
        )
        DB_PATH.unlink()
