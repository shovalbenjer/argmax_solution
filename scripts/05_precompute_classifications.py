...
The script performs comprehensive pre-computation including:
- Ingredient classification processing from nutrition database
- Recipe classification processing from OpenSearch
- Redis cache storage with appropriate TTL values
...
Raises:
    ...
    ConnectionError: If Redis or database connections fail
    Exception: If pre-computation processing fails
"""
import argparse
import asyncio
import json
...
class ClassificationPrecomputer:
    ...
    - Ingredient classification pre-computation
    - Recipe classification pre-computation
    ...
    def __init__(self, batch_size: int = 100):
        """
        Initialize the ClassificationPrecomputer.
    ...
    def get_all_ingredients(self) -> List[str]:
        ...
        This function queries the nutrition_facts table to retrieve all
        unique ingredient names for pre-computation processing.
        
        Returns:
            List[str]: List of unique ingredient names
            
        Raises:
            sqlite3.Error: If database query fails
            Exception: If ingredient loading fails
        """
        ...
        try:
            with sqlite3.connect(app_config.DB_PATH) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT DISTINCT name FROM nutrition_facts WHERE name IS NOT NULL AND name != ''"
                )
    ...
    def get_sample_recipes(self, limit: int = 1000) -> List[Dict[str, Any]]:
        ...
        This function queries OpenSearch to retrieve sample recipes with
        ingredient data for pre-computation processing.
        
        Args:
            limit (int): Maximum number of recipes to retrieve
            
        Returns:
            List[Dict[str, Any]]: List of recipe dictionaries with id, title, and ingredients
            
        Raises:
            Exception: If OpenSearch query fails
        """
        ...
        try:
            ...
            if not client:
                logger.warning(
                    "OpenSearch not available - skipping recipe pre-computation"
                )
                return []
    ...
    async def classify_ingredient_async(
        self, ingredient: str
    ) -> Optional[Dict[str, Any]]:
        """
        Classify a single ingredient asynchronously.
        
        This function performs keto and vegan classification for a single
        ingredient and returns detailed results with timing information.
        
        Args:
            ingredient (str): Ingredient name to classify
    ...
    async def classify_recipe_async(
        ...
        This function performs keto and vegan classification for a recipe
        containing multiple ingredients and returns detailed results.
        
        Args:
            recipe (Dict[str, Any]): Recipe dictionary with id, title, and ingredients
    ...
    def precompute_ingredients(self, ingredients: List[str]) -> int:
        ...
        This function processes ingredients in batches, performing classification
        and caching results in Redis for fast runtime access.
        
        Args:
            ingredients (List[str]): List of ingredient names to process
            
        Returns:
            int: Number of successfully cached ingredients
            
        Raises:
            Exception: If batch processing fails
        """
        logger.info(
            f"Pre-computing classifications for {len(ingredients)} ingredients..."
        )

        if not self.cache_manager.is_available():
            logger.warning(
                "Redis cache not available - classifications will not be cached"
            )
            return 0
        ...
        for i in range(0, len(ingredients), self.batch_size):
            ...
            logger.info(
                f"Processing batch {i//self.batch_size + 1}: ingredients {i+1}-{min(i+len(batch), len(ingredients))}"
            )

            # Process batch concurrently
            async def process_batch():
                tasks = [
                    self.classify_ingredient_async(ingredient) for ingredient in batch
                ]
            ...
            try:
                ...
                for result in results:
                    if result and not isinstance(result, Exception):
                        ...
                        self.cache_manager.set_ingredient_context(
                            result["ingredient"], result, ttl=604800  # 1 week
                        )
                        cached_count += 1
                ...
                logger.info(
                    f"  Batch completed in {batch_time:.2f}s - {len(batch)} ingredients processed"
                )
    ...
    def precompute_recipes(self, recipes: List[Dict[str, Any]]) -> int:
        ...
        This function processes recipes in batches, performing classification
        and caching results in Redis for fast runtime access.
        
        Args:
            recipes (List[Dict[str, Any]]): List of recipe dictionaries to process
            
        Returns:
            int: Number of successfully cached recipes
        ...
        for i in range(0, len(recipes), recipe_batch_size):
            ...
            logger.info(
                f"Processing recipe batch {i//recipe_batch_size + 1}: recipes {i+1}-{min(i+len(batch), len(recipes))}"
            )
            ...
            for recipe in batch:
                ...
                if result:
                    ...
                    self.cache_manager.set_classification_result(
                        result["recipe_id"], result, ttl=86400  # 1 day
                    )
                    cached_count += 1
                    self.processed_count += 1
            ...
            batch_time = time.time() - batch_start
            logger.info(
                f"  Recipe batch completed in {batch_time:.2f}s - {len(batch)} recipes processed"
            )
    ...
    def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        This function retrieves comprehensive statistics about the Redis cache
        including key counts, memory usage, and status information.
...
def main():
    ...
    This function parses command-line arguments and orchestrates the complete
    pre-computation pipeline for ingredients and recipes.
    ...
    parser = argparse.ArgumentParser(
        description="Pre-compute diet classifications for caching"
    )
    parser.add_argument(
        "--ingredients-only",
        action="store_true",
    ...
    if not cache_manager.is_available() and not args.dry_run:
        ...
        logger.info(
            "To start Redis with Docker: docker run -d -p 6379:6379 redis:alpine"
        )
        return 1
    ...
    try:
        ...
        if not args.dry_run:
            ...
            if cache_stats.get("status") == "connected":
                ...
                logger.info(
                    f"Cache memory usage: {cache_stats.get('memory_usage', 'unknown')}"
                )

            success_rate = (
                (precomputer.cached_count / precomputer.processed_count * 100)
                if precomputer.processed_count > 0
                else 0
            )
