...
The script performs comprehensive performance testing including:
- Cold vs warm performance measurement (cache impact analysis)
- Throughput testing (recipes/second processing rate)
- Latency measurement (average and 95th percentile)
- Memory usage monitoring during processing
- Cache hit rate validation and Redis performance testing
- Input format performance comparison
...
Raises:
    ...
    Exception: If performance testing fails
"""
import json
import logging
import os
...
def generate_test_recipes(count=50):
    ...
    This function creates a diverse set of test recipes with different
    ingredient combinations to simulate real-world usage patterns.
    
    Args:
        count (int): Number of test recipes to generate
        
    Returns:
        list: List of recipe dictionaries with id and ingredients
    """
    ...
    for i in range(count):
        if i % 3 == 0:
            # Keto recipe
            ingredients = keto_ingredients[:3] + [
                keto_ingredients[i % len(keto_ingredients)]
            ]
        elif i % 3 == 1:
            # Vegan recipe
            ingredients = vegan_ingredients[:3] + [
                vegan_ingredients[i % len(vegan_ingredients)]
            ]
        else:
            # Mixed recipe
            ingredients = mixed_ingredients[:3] + [
                mixed_ingredients[i % len(mixed_ingredients)]
            ]

        recipes.append({"id": f"test_recipe_{i}", "ingredients": ingredients})
...
def measure_recipe_classification_performance():
    ...
    This function performs comprehensive performance testing including:
    - Cold performance measurement (first-time processing)
    - Warm performance measurement (cache-hit processing)
    - Throughput calculation (recipes per second)
    ...
    Raises:
        ...
        Exception: If performance measurement fails
    """
    ...
    cache_hit_rate = (
        cache_hits / len(test_recipes[:10]) if len(test_recipes[:10]) > 0 else 0
    )
    ...
    return {
        ...
        "error_rate": (errors + batch_errors)
        / (len(test_recipes[:10]) * 2 + len(batch_recipes)),
...
def measure_memory_usage():
    ...
    This function monitors memory consumption during recipe classification
    to ensure the system operates within acceptable memory limits.
    ...
    Raises:
        ImportError: If diet_classifiers module cannot be imported
        Exception: If memory measurement fails
    """
    ...
    avg_memory = (
        statistics.mean(memory_measurements) if memory_measurements else baseline_memory
    )
...
def test_input_format_performance():
    ...
    This function compares performance across different input formats
    (JSON, CSV, List) to identify any performance bottlenecks related
    to input parsing.
    
    Returns:
        dict: Dictionary containing format-specific performance metrics
        
    Raises:
        ImportError: If diet_classifiers module cannot be imported
        Exception: If format testing fails
    """
    from diet_classifiers import is_keto, is_vegan
...
def test_cache_performance():
    ...
    This function validates Redis cache functionality and provides
    statistics on cache usage and performance.
    
    Returns:
        dict: Dictionary containing cache performance metrics
        
    Raises:
        ImportError: If cache manager module cannot be imported
        Exception: If cache testing fails
    """
...
def main():
    ...
    This function orchestrates the complete performance benchmarking pipeline:
    1. Recipe classification performance measurement
    2. Memory usage monitoring
    3. Input format performance testing
    4. Cache performance validation
    5. Benchmark evaluation and reporting
    ...
    try:
        ...
        logger.info(
            f"{'PASS' if throughput_met else 'WARN'}: Throughput: {all_results['recipes_per_second']:.2f} recipes/sec "
        ...
        # Latency benchmarks
        avg_latency_met = (
            all_results["avg_recipe_latency_ms"] <= BENCHMARKS["max_avg_latency_ms"]
        )
        p95_latency_met = (
            all_results["p95_recipe_latency_ms"] <= BENCHMARKS["max_p95_latency_ms"]
        )

        logger.info(
            f"{'PASS' if avg_latency_met else 'WARN'}: Avg Latency: {all_results['avg_recipe_latency_ms']:.1f}ms "
            f"(benchmark: {BENCHMARKS['max_avg_latency_ms']:.1f}ms)"
        )
        logger.info(
            f"{'PASS' if p95_latency_met else 'WARN'}: P95 Latency: {all_results['p95_recipe_latency_ms']:.1f}ms "
        ...
        logger.info(
            f"{'PASS' if memory_met else 'WARN'}: Peak Memory: {all_results['peak_memory_gb']:.2f}GB "
        ...
        logger.info(
            f"{'PASS' if error_rate_met else 'WARN'}: Error Rate: {all_results['error_rate']:.3f} "
        ...
        if all_results.get("cache_available"):
            cache_hit_met = (
                all_results["cache_hit_rate"] >= BENCHMARKS["cache_hit_rate_min"]
            )
            logger.info(
                f"{'PASS' if cache_hit_met else 'INFO'}: Cache Hit Rate: {all_results['cache_hit_rate']:.3f} "
        ...
        logger.info("\nPerformance Insights:")
        logger.info(
            f"  Cold vs Warm Latency: {all_results.get('cold_avg_latency_ms', 0):.1f}ms vs {all_results.get('warm_avg_latency_ms', 0):.1f}ms"
        )
        logger.info(
            f"  Memory Usage: {all_results['memory_increase_gb']:.2f}GB increase during processing"
        )
