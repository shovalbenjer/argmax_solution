...
def validate_database_exists():
    ...
    if size_mb < BENCHMARKS['min_database_size_mb']:
        logger.warning(f"Database size {size_mb:.1f} MB below benchmark {BENCHMARKS['min_database_size_mb']} MB")
    else:
        logger.info(f"Database size meets benchmark ({size_mb:.1f} MB >= {BENCHMARKS['min_database_size_mb']} MB)")
...
def validate_record_counts():
    ...
    with sqlite3.connect(app_config.DB_PATH) as conn:
        ...
        if nutrition_count < BENCHMARKS['nutrition_facts_min_records']:
            logger.warning(f"nutrition_facts: {nutrition_count} records < {BENCHMARKS['nutrition_facts_min_records']} benchmark")
        else:
            logger.info(f"nutrition_facts: {nutrition_count} records >= {BENCHMARKS['nutrition_facts_min_records']} benchmark")
        ...
        if vegan_count < BENCHMARKS['vegan_ontology_min_records']:
            logger.warning(f"vegan_ontology: {vegan_count} records < {BENCHMARKS['vegan_ontology_min_records']} benchmark")
        else:
            logger.info(f"vegan_ontology: {vegan_count} records >= {BENCHMARKS['vegan_ontology_min_records']} benchmark")
        ...
        if unit_count < BENCHMARKS['unit_conversions_min_records']:
            logger.warning(f"unit_conversions: {unit_count} records < {BENCHMARKS['unit_conversions_min_records']} benchmark")
        else:
            logger.info(f"unit_conversions: {unit_count} records >= {BENCHMARKS['unit_conversions_min_records']} benchmark")
...
def validate_data_quality():
    ...
    with sqlite3.connect(app_config.DB_PATH) as conn:
        ...
        cursor.execute("SELECT COUNT(*) FROM nutrition_facts WHERE name IS NULL OR name = '';")
        null_names = cursor.fetchone()[0]
        
        if null_names > 0:
            logger.warning(f"Found {null_names} nutrition records with null/empty names")
        else:
            logger.info("No null ingredient names found")
        
        # Check for duplicate ingredient names
        cursor.execute("SELECT name, COUNT(*) FROM nutrition_facts GROUP BY name HAVING COUNT(*) > 1;")
        ...
        calories_percentage = (valid_calories / total_records * 100) if total_records > 0 else 0
        logger.info(f"Records with valid calories: {valid_calories}/{total_records} ({calories_percentage:.1f}%)")
...
def main():
    ...
    try:
        ...
        for table, count in record_counts.items():
            ...
            logger.info(f"{status}: {table}: {count:,} records (benchmark: {benchmark:,})")
