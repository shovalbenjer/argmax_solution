...
def select2():
    ...
    return jsonify(
        {
            "results": results,
            "pagination": {"more": end_idx < len(matching_ingredients)},
        }
    )
...
def search_by_ingredients():
    ...
    if not ingredient_query:
        return jsonify({"error": "No ingredients provided", "recipes": [], "total": 0})
    ...
    try:
        ...
        return jsonify({"error": "Search failed", "recipes": [], "total": 0})
